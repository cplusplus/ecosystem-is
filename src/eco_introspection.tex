...
\pnum
\begin{outputblock}
@\emph{application}@ [--std-info[=@\grammarterm{declaration}@]] [--std-info-out=@\emph{file}@]
\end{outputblock}
...
\pnum
\verb@--std-info=@\grammarterm{declaration}
\begin{indented}
...
\nontermdef{minor-patch-part}\br
	\ucode{002E} \uname{FULL STOP} minor-number \opt{patch-part}

\nontermdef{patch-part}\br
	\ucode{002E} \uname{FULL STOP} patch-number
...
\nontermdef{decimal}\br
	\descr{one  of:} \ucode{0030} .. \ucode{0039} \uname{DIGIT ONE .. NINE} \br
	\descr{zero or more of:} \ucode{0030} .. \ucode{0039} \uname{DIGIT ZERO .. NINE}

\end{ncbnf}

\pnum
A \grammarterm{decimal} is a sequence of decimal digits
(\ucode{0030} .. \ucode{0039} \uname{DIGIT ZERO .. NINE})
with no superfluous leading zeros (\ucode{0030} \uname{DIGIT ZERO}).

\pnum
A \grammarterm{decimal} represents a non-negative base 10 integer.

\pnum
A version number is composed of 1, 2, or 3 decimal numbers
(\grammarterm{decimal}) separated by a \ucode{002E} \uname{FULL STOP}.
...
or more \verb|--std-info=|\grammarterm{declaration} options shall conform its
...
\pnum
An application, when not given a \verb|--std-info=|\grammarterm{declaration}
